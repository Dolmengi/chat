...
public class ChatChannelAdapter implements ChatChannelPort {
    ...
    @Override
    public ChatChannel createChannel(ChatGroup chatGroup, String name, ChannelType type) {
        ChatGroupEntity groupEntity = groupRepository.findById(chatGroup.id()).orElseThrow(() -> new ChatException(INVALID_PARAMETER));
        ChatChannelEntity entity = ChatChannelEntity.builder()
                .chatGroup(groupEntity)
                .name(name)
    ...
    @Override
    public ChatChannel updateChannel(Long id, String name, ChannelType type) {
        ChatChannelEntity entity = repository.findById(id).orElseThrow(() -> new ChatException(INVALID_PARAMETER));
